...
class Crawler
  ...
  def parse_ak(h)
    ...
    h[:tested] = 2894+2128  # from image! save image?
    if @auto_flag
...
byebug
  ...
  def parse_ct(h)
    ...
    if @s =~ /([^'"]+CTDPHCOVID19summary[^'"]+)/
      ...
      if result =~ /\.([^\.]+) residents have died/
        h[:deaths] = string_to_i($1.strip)
      else
        @errors << 'missing deaths'
      end
      result = reader.page(7).text.gsub(/\s+/,' ').gsub(',','') 
      if result =~ /more than ([\d]+) tests have been reported/
        h[:tested] = string_to_i($1)
      else
        @errors << 'missing tested'
      end
...
# County is available
  ...
  def parse_nc(h)
    crawl_page
    @s = @doc.css('table')[0].text.gsub(/\s+/,' ').gsub(',','')
    if @s =~ /Cases Deaths Completed Tests Currently Hospitalized Number of Counties (\d+) (\d+) (\d+) (\d+) (\d+)/
      h[:tested] = $3.to_i
      h[:positive] = $1.to_i
...
# TODO get counties
  ...
  def parse_sd(h)
    ...
    if (t = tables.select {|i| i=~/COVID-19 IN SOUTH DAKOTA/}[0]) &&
      ...
      t =~ /Cases\s(\d+)\sHospitalized\s(\d+)\sDeaths\s(\d+)\sRecovered\s(\d+)\s/
      h[:hospitalzed] = string_to_i($2)
      h[:deaths] = string_to_i($3)
      h[:recovered] = string_to_i($4)
    else
      @errors << "missing deaths"
    end
    h
...
=end
    ...
    h[:tested] = string_to_i(numbers[0][0])
    h[:positive] = string_to_i(numbers[1][0])
    h[:hospitalized] = string_to_i(numbers[2][0])
    h[:deaths] = string_to_i(numbers[3][0])  
    h
  end
    ...
    loop do
      begin
        ...
        if @s
          @s.gsub!("\n",'|')
          @s.gsub!(/\s+/,' ')
          if @s =~ /Cases\| Total Deaths\| (\d+)\|(\d+)/
            h[:positive] = $1.to_i
            h[:deaths] = $2.to_i
            if @s =~ /Total tests\| Percent Positive\| (\d+)/
              h[:tested] = $1.to_i
              break
            end
          end
