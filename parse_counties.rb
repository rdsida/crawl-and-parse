...
h_state = {}
open('counties.csv').readlines.map {|i| i.strip.split("\t")}.each {|st, url| h_state[st.upcase] = url}
  ...
  for line in open(file).readlines
    ...
    if positive
      if h_latest[:positive] && h_latest[:positive].to_i > positive.to_i
        ...
        puts [_st, ts, tested, positive, deaths].join("\t") if positive.to_i > h_latest[:positive].to_i
...
lines = open('all_counties.csv').readlines
lines.shift
lines.map {|i| i.split("\t")}.each do |st, tested, positive, deaths, junk| 
  puts st
  st2, tested2, positive2, deaths2, junk = arr[j].split("\t") rescue next
...
open('all_counties.csv','w') do |fout|
